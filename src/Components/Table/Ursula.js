...
function getUrsula(cpf, getTimeline) {
  let awsConfig2 = {
...
export default function Ursula() {
  ...
  const {changeActive, row, timeline, getTimeline} = useUsersContext();

  useEffect(() => {
    getUrsula(row.cpf, getTimeline);
    getAllUrsula();
