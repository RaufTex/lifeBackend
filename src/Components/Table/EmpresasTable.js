...
export default function EmpresasTable() {
  ...
  const { changeActive, getRow, users, getUsers, getAddress } = useUsersContext();
  ...
  const rows = empresas?.map((result) => {
    return createData(
      result.nomeEmpresa.S,
      result.createdAt.S,
      result.cnpj.S,
      result.email.S,
      "info",
      ...
      <Paper sx={{ width: "100%" }}>
        ...
        <Box sx={{ display: "flex", flexDirection: "row", marginBottom: "2%" }}>
          ...
          <Box sx={{ marginLeft: "10%" }}>
            ...
            />
            <TextField
                sx={{ width: "220px", marginLeft: "10px", '@media screen and (max-width: 1240px)': {
                  width: "180px",
                } }}
                id="outlined-search"
            ...
            <TableBody>
              ...
              {stableSort(rows, getComparator(order, orderBy))
                ...
                .filter((row) => {
                  if (setSearchName === "") {
                    return row;
                  } else if (row.name.toLowerCase().includes(searchName.toLowerCase())) {
                    return row;
                  }
                })
